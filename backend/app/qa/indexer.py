...
# Sections that contain no useful QA content
_LOW_VALUE_SECTION_RE = re.compile(
    r"(?i)^(references?|acknowledg?ments?|ccs\s+concepts|keywords?|acm\s+reference|declarations?)\b"
)

# Max chunks from a single paper in search results (diversity cap)
_MAX_CHUNKS_PER_PAPER = 3


def _get_collection():
    client = chromadb.PersistentClient(path=settings.qa_chroma_dir)
...
def chunk_paper(md_content: str, paper_id: str, paper_title: str, max_chunk_chars: int = 800, overlap_chars: int = 100) -> list[dict]:
    """Split paper content into chunks, preferring section boundaries. Skips low-value sections."""
    sections = re.split(r"\n(?=## )", md_content)
    chunks = []
    ...
    for section in sections:
        ...
        section_title = title_match.group(1).strip() if title_match else chr(27491)+chr(25991)
...
async def index_paper(paper_id: str, paper_title: str, md_content: str):
    """Chunk and index a single paper into ChromaDB."""
...
def delete_paper_index(paper_id: str):
    """Delete all chunks belonging to a paper from ChromaDB."""
    collection = _get_collection()
    try:
...
async def rebuild_all_papers(db_papers: list[dict]):
    """Rebuild entire index. db_papers should be list of dicts with id, title, md_content."""
    collection = _get_collection()
    try:
    ...
    for j in range(0, len(texts), embed_batch):
        ...
        embeddings.extend(await get_embeddings(sub_texts))
...
def search_chunks(query_embedding: list[float], top_k: int = 8) -> list[dict]:
    """Search for most relevant chunks, with per-paper deduplication and relevance filtering."""
    collection = _get_collection()
    ...
    if results and results["ids"] and results["ids"][0]:
        for i, chunk_id in enumerate(results["ids"][0]):
            ...
            cnt = paper_counts.get(paper_id, 0)
            if cnt >= _MAX_CHUNKS_PER_PAPER:
                continue
            paper_counts[paper_id] = cnt + 1
