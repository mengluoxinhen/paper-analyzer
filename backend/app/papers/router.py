...
async def rename_folder(folder_id: int, body: schemas.FolderCreate, db: AsyncSession = DBSession):
...
async def delete_folder(folder_id: int, db: AsyncSession = DBSession):
...
async def delete_tag(tag_id: int, db: AsyncSession = DBSession):
...
async def upload_paper(
    ...
    folder_id: int = Query(default=None),
    ...
    md_content = ""
    if md_file:
        md_filename = md_file.filename or f"{filename}.md"
        md_upload_dir = utils.ensure_upload_dir("papers")
        safe_md = f"{ts}_{md_filename}"
        md_path = os.path.join(md_upload_dir, safe_md)
        md_bytes = await md_file.read()
        async with aiofiles.open(md_path, "wb") as f:
            await f.write(md_bytes)
        md_content = md_bytes.decode("utf-8", errors="replace")

    status = "parsed" if md_content else "uploaded"
    ...
    return crud.paper_to_response(paper)
    return crud.paper_to_response(paper)


@router.get("/{paper_id}/pdf")
async def serve_pdf(paper_id: int, db: AsyncSession = DBSession):
...
async def list_papers(
    ...
    papers, total = await crud.get_papers(db, page, page_size, keyword, folder_id, tag)
    return {
        "papers": [crud.paper_to_response(p) for p in papers],
        "total": total,
    }


@router.get("/{paper_id}", response_model=schemas.PaperDetail)
async def get_paper(paper_id: int, db: AsyncSession = DBSession):
    paper = await crud.get_paper(db, paper_id)
    if not paper:
        raise HTTPException(404, "论文不存在")
    result = crud.paper_to_response(paper)
    result["md_content"] = paper.md_content or ""
    return result


@router.put("/{paper_id}", response_model=schemas.PaperResponse)
async def update_paper(paper_id: int, body: schemas.PaperCreate, db: AsyncSession = DBSession):
...
async def delete_paper(paper_id: int, db: AsyncSession = DBSession):
    success = await crud.delete_paper(db, paper_id)
    if not success:
        raise HTTPException(404, "论文不存在")
    return {"ok": True}
...
async def move_paper(paper_id: int, body: schemas.PaperMoveRequest, db: AsyncSession = DBSession):
    try:
...
async def set_paper_tags(paper_id: int, body: schemas.PaperTagUpdate, db: AsyncSession = DBSession):
...
async def summarize_paper(paper_id: int, db: AsyncSession = DBSession):
    paper = await crud.get_paper(db, paper_id)
    if not paper:
        raise HTTPException(404, "论文不存在")
...
async def get_summary(paper_id: int, db: AsyncSession = DBSession):
    summary = await crud.get_summary(db, paper_id)
    return summary


@router.post("/{paper_id}/chat")
async def chat_with_paper(paper_id: int, body: schemas.ChatRequest, db: AsyncSession = DBSession):
    paper = await crud.get_paper(db, paper_id)
    if not paper:
        raise HTTPException(404, "论文不存在")
...
async def extract_paper_info(paper_id: int, body: schemas.ExtractRequest, db: AsyncSession = DBSession):
    paper = await crud.get_paper(db, paper_id)
    if not paper:
        raise HTTPException(404, "论文不存在")
...
async def get_extractions(paper_id: int, db: AsyncSession = DBSession):
