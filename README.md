...
基于 AI 的论文学术助手，上传 PDF 后自动调用 MinerU 解析为 Markdown，支持 AI 总结、多轮对话问答（RAG 检索增强）、全局知识库问答、文件夹管理与标签分类。

## ✨ 功能

- **📤 PDF 上传与解析** — 拖拽上传 PDF，自动调用 MinerU v4 API 解析为结构化 Markdown，流式进度展示
- **🤖 AI 自动总结** — 解析完成后自动生成结构化总结（解决的问题、结论、工况表格），支持重新生成
- **💬 论文对话** — 基于论文全文的多轮 AI 问答，流式输出，支持多会话管理（新建/切换/删除对话历史）
- **🌐 全局问答** — 基于所有论文知识库的 RAG 检索增强问答，向量语义搜索 + LLM 综合回答
